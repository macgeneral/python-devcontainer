#!/bin/sh
# Optional Hardening
# - based on https://github.com/ironpeakservices/iron-alpine/blob/master/Dockerfile
...
sed -i -r "/^(${GROUP}|root|nobody)/!d" /etc/group
...
# Remove apk configs.
find /bin /etc /lib /sbin /usr -xdev -type f -regex '.*apk.*' ! -name apk -exec rm -fr {} +
